<template>
          ...
          <div class="row login-register-cover">
            <div class="col-lg-4 col-md-6 col-sm-12 mx-auto">
              <div class="text-center">
                ...
                <h2 class="mt-10 mb-5 text-brand-1">Authentification pour les recruteurs</h2>
              ...
              <form class="login-register text-start mt-20" @submit.prevent="login">
                <div class="form-group">
                  <label class="form-label" for="input-1">Adresse Email Pro*</label>
                  <input class="form-control" id="input-1" type="email" v-model="email" required placeholder="Foulenbenfoulen@gmail.com">
                </div>
                <div class="form-group">
                  <label class="form-label" for="input-4">Mot de passe *</label>
                  <input class="form-control" id="input-4" type="password" v-model="password" required placeholder="************">
                </div>
                <div class="login_footer form-group d-flex justify-content-between">
                  <label class="cb-container">
                    <input type="checkbox"><span class="text-small">Se souvenir de moi</span>
                  </label>
                  <a class='text-muted' href='page-contact.html'>Mot de passe oublié ?</a>
                </div>
                <div class="form-group">
  ...
  const login = async () => {
    try {
      const response = await axios.post('http://localhost:8000/api/societe/login', {
        email: email.value,
