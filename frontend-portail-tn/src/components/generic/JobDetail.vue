    ...
    <section class="section-box-2">
      <div class="container">
        <div class="banner-hero banner-image-single">
          <img
            src="/assets/home/imgs/page/job-single/thumb.png"
            alt="PortailTN"
          />
        </div>
        <div class="row mt-10">
        ...
        <div class="row">
          <div class="col-lg-8 col-md-12 col-sm-12 col-12">
            ...
            <div class="content-single">
              ...
              <p
                class="ml-20"
                v-if="JSON.parse(job.skills || '[]').length === 0"
              >
...
const postuler = async (offreId) => {
  ...
  try {
    const response = await axios.post(
      `http://localhost:8000/api/postulation/${offreId}/${userId.value}`
    );
...
const fetchUserApplications = async () => {
  try {
    const response = await axios.get(
      `http://localhost:8000/api/postulation/user/${userId.value}`
    );
...
const alreadyApplied = () => {
  return applications.value.some(
    (application) => application.offre_id === props.jobId
  );
};
